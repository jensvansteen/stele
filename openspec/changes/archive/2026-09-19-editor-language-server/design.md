...
See proposal.md for the motivation. This change adds `stele lsp` to the existing binary. It was first planned before `link-index` merged; this revision plans against `main` at 0.1.0-rc.4, where `link-index`, `spec-annotation`, and `terminal-report` are merged and archived, and, since the re-plan of section 6, against `fast-runs` as merged on `main` (`38a0349`, batching only) for running tests.
...
| `fast-runs` (batching only): `runScopeTests(testRequest)` with targets and merge, the `runTestGroups` seam (`executeGroupsInBatches`, with `executeGroupsOneByOne` and `executeExactTest` as the per-test reference), sequential batches (one process per Go package and build tag set, or per Node file), `testObserver` plus the optional `batchObserver.batchEnded`, `testRun.incomplete`, and the `test-process-failed` reason for tests of a batch that did not complete | **merged** (`38a0349`, 2026-09-18) | `stele.runTests`, its progress, and the incomplete-batch report |
| Not in `fast-runs` and deferred there: a run context, interruption (Ctrl-C) handling, process groups, `--jobs`, `execution.jobs`, `execution.groups`, group setup and teardown, worker identity (`STELE_WORKER_ID`, `STELE_JOBS`, `STELE_GROUP`), timeouts | **not built** | Cancellation is owned by the server (Decision 8); the rest is deferred with scenario `cbace7f5db1a` (see Deferred) |

**The run feature builds on the merged batching runner.** `fast-runs` merged on 2026-09-18 trimmed to sequential batching: it has no run context, no interruption handling, no parallelism, and no execution groups. Section 6 therefore uses the runner as it is and adds only the cancellation the editor needs, on the server's own path (Decision 8). Everything that depended on parallel execution is listed under "Deferred".
...
**Status: re-planned on 2026-09-18 against the merged batching runner (`38a0349`); approved by jensvansteen on 2026-09-18 (via: cli).**

- **One runner.** `stele.runTests` validates the targets against the index (an unknown target gives JSON-RPC error `-32602` naming it, and nothing runs), then calls the same entry point as `stele test <targets...>`: `runScopeTests` with `targets`, `merge`, and the evidence path, in a goroutine, one call per affected scope in scope order. The server therefore gets the same selection, the same batches (one process per Go package and build tag set, or per Node file, one batch after another), the same per-test results, and the same merge without code of its own. There is no `--jobs`, no `execution.groups`, and no worker identity to pass: none exists on `main`.
- **Cancellation, owned by the server.** The runner has no run context, and the CLI does not handle Ctrl-C. The server adds the smallest seam it needs:
  - `testRequest` gains an optional `cancel` context. The CLI never sets it, so `stele test`, `validate`, and `check` behave exactly as today (no process group, no check between batches, Ctrl-C as before: the terminal signals the whole foreground process group).
  - When the context is set, each batch command starts in its own process group (`SysProcAttr.Setpgid`), because `go test` runs the compiled test binary as a child and killing only the `go` process would orphan it. On cancellation the server sends `SIGTERM` to the group, then `SIGKILL` about 2 seconds later if the batch process is still running (decided by jensvansteen on 2026-09-18, via chat), the batch loop starts no further batch, and `runScopeTests` returns `errRunCancelled` before it assembles or writes evidence, so the stored file keeps its bytes.
  - `$/cancelRequest` for the command and `window/workDoneProgress/cancel` for its token cancel the context. The command ends with the LSP `RequestCancelled` error (`-32800`), and the progress ends with "cancelled".
  - Both platforms Stele ships for (darwin, linux) support process groups; Windows is not a packaged target.
...
- **Progress.** When the request carries a `workDoneToken`, the server uses it; otherwise, when the client declares `window.workDoneProgress`, it creates one with `window/workDoneProgress/create`. A progress adapter implements `testObserver` and `batchObserver`:
  - `planned(total, …)` → `begin` with title `Stele: running <total> tests`, `cancellable: true`, `percentage: 0`;
  - `finished(event)` → `report` with `<run>/<total> · <passed> passed · <failed> failed`, the percentage, and `failed: <title> (<level>)` when that test failed; batches run one after another, so there is never more than one batch in flight;
  - `batchEnded(event)` for a batch that did not complete → `report` with `incompleteText` (`<batch> did not complete (<exit status>)`), the same words as the terminal report; a completed batch sends nothing extra;
  - the end of the run → `end` with the summary line, or "cancelled".

  Progress messages follow the order the runner reports; the result is built from the sorted executions.
- **Result.** `{ "executions": [{ "evidenceId", "path", "line", "selector", "outcome", "reason" }], "incomplete": [{ "batch", "status", "unreported" }], "scopes": [{ "scope", "verdicts": { "linkage", "execution", "overall" } }] }`, sorted by path and selector, by batch label, and by scope order. `incomplete` comes from `testRun.incomplete`; its tests appear in `executions` with the reason `test-process-failed`, as in the evidence. Verdicts come from `verifyLoaded` and `reportVerdicts` over the evidence after the run at the scope's stage (Decision 5), so a green run in a scope with a linkage error reads `overall: fail`, as in `stele verify`.
- **After a run** the evidence file is written by the runner's writer, which the watcher observes like any other change; the server also rebuilds directly and sends `workspace/codeLens/refresh` and diagnostics, so the result and the lenses agree without waiting for the watcher.
- **Run lenses.** With `stele.runTests` in place, the CodeLens items of Decision 6 gain their run actions (`▶ Run all`, one per level present, `▶ Run` on test anchors) in the fixed order summary, run all, `unit`, `integration`, `e2e`, status. Their arguments are `stele test` targets. This is what scenarios `f584d3ec6c44`, `f1cf94d0c1d6`, and `cdca0ea9f34c` verify; their text and approved entries are unchanged.
...
**Status: approved by jensvansteen on 2026-09-18, before implementation started (via: cli).** The 47 entries of that approval were approved in `linkage-plan.json`. **Re-plan of 2026-09-18 after `fast-runs` merged, approved by jensvansteen on 2026-09-18 (via: cli):** `….3c56123372c1.unit` is replaced by `….3c56123372c1.integration` for the reworded cancel scenario, `….40c7e3ae9243.unit` is new, and `….cbace7f5db1a.unit` is removed with its scenario (see Deferred). Every other entry, including the six other run scenarios and the three run-lens scenarios, keeps its text, level, and approval. This revision keeps the 31 entries of the first draft (five with reworded scenarios, marked "reworded") and adds 16 entries for the new scenarios (marked "new").

Placement follows this repository's AGENTS.md: co-located Go tests in `internal/stele` (the `lsp_*_test.go` files beside each `lsp_*.go` file), and executable contract tests of the shipped binary in `tests/cli.test.mts`. Placement is advisory. Unit tests drive the server in process through `io.Pipe` with fixture repositories in temporary directories, a recording stub for the runner's batch and group-command seams (from `fast-runs`), and an injected clock. e2e is used only where the shipped binary adds a distinct risk: the real stdio framing, process lifecycle, and build-time version, and byte equality with the CLI. One integration entry is planned, for cancellation: killing a real process tree is the one runner behavior this change adds and that no earlier evidence covers. Otherwise real test processes are covered by the runner's own evidence in `execution`, `go-support`, and `fast-runs`.
...
| `scn.languageserver.3c56123372c1` Cancel a running command (reworded 2026-09-18: a batch process and its child, no further batch; approved in the re-plan) | integration | `….3c56123372c1.integration` (replaces `….unit`) | `internal/stele/lsp_run_test.go`, with a helper test process that starts a child and blocks until killed | Risk: an orphaned test binary (`go test` runs it as a child), a batch that still starts after cancel, or a half-written evidence file. Only a real process tree started through the batch runner, cancelled mid-run, shows the group is gone and the evidence bytes are unchanged; no editor is involved. |
...
| `scn.languageserver.40c7e3ae9243` Report a batch that did not complete (new 2026-09-18, approved in the re-plan) | unit | `….40c7e3ae9243.unit` | `internal/stele/lsp_run_test.go`, with a fake batch process (`startBatchProcess`) and an in-process session | Risk: a crashed batch reads as a clean run in the editor, or its unreported test is dropped. The fake output stream stops after one result, deterministically; real crash handling is covered by `fast-runs`' evidence. |
...
- [The runner has no cancellation of its own] → The server adds an opt-in `cancel` context and a process group only on its own path; the CLI path does not set them and is unchanged. When the CLI gets interruption handling (deferred in `fast-runs`), both can share it.
...
- [Two runs from CLI and editor at once] → The server serializes its own runs; the atomic evidence writer plus merge by evidence ID keeps concurrent CLI runs from corrupting the file. The last writer wins per evidence entry, which is documented. Batches share nothing between runs, so two sequential runs from the editor and a terminal only race on the evidence file, which the merge handles.
...
### Re-plan after `fast-runs` merged (2026-09-18)

`fast-runs` merged as batching only (`38a0349`). Section 6, Decision 8, and the run lenses are re-planned against it (approved by jensvansteen on 2026-09-18, via cli): the run goes through `runScopeTests` and the batching runner; progress maps the existing observer and `batchEnded` to `$/progress`; batches that did not complete are reported; the server owns cancellation; one run per project. The requirement "Run selected tests and merge their results" no longer names job counts, execution groups, or setup and teardown. Scenario `3c56123372c1` is reworded, `40c7e3ae9243` is new, and `cbace7f5db1a` is removed. Section 6 and the run lenses were implemented on 2026-09-18 after this approval.

## Deferred (future change)

Deferred together with the parallel execution that `fast-runs` deferred:

- Scenario `scn.languageserver.cbace7f5db1a` "Use the project's execution settings" and its plan entry `….cbace7f5db1a.unit` (removed from this change): runs that honor `execution.jobs`, `execution.groups`, group setup and teardown, and worker identity exactly as `stele test` does.
- Progress for several tests in flight (`· <n> running`) and for group setup and teardown.
- Teardown on cancellation, and reporting a teardown failure in the run result.
- Sharing interruption handling with the CLI once `stele test` handles Ctrl-C.
...
For the re-plan of section 6, decided by jensvansteen on 2026-09-18 (via chat):

7. **How cancellation stops the process tree.** Decided: `SIGTERM` to the batch's process group, then `SIGKILL` after about 2 seconds if the batch process is still running.
8. **The CLI and interruption.** Decided: leave `stele test` as it is (no process group, no Ctrl-C handling), because `fast-runs` deferred that; the server's `cancel` context stays opt-in. The CLI does not adopt the cancel context now.
9. **A target declared in several scopes.** A requirement modified by a change exists in the current specifications and in the change. Decided, as proposed: a lens runs in the scope of its own heading or anchor, and a command target that several scopes declare runs once, in the first scope in scope order, so a test never runs twice in one command.
