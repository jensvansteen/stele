...
- **Running tests**: run buttons execute through the same runner as `stele test <targets...>`, with its selection, merging, and the sequential batching that the merged `fast-runs` change added. Progress is reported with `$/progress`, including batches that did not complete; the server cancels a run itself by stopping the running batch's process group and writing no evidence. The result names each outcome, the incomplete batches, and the scope's `verdicts` (`linkage`, `execution`, `overall`). Parallel runs and execution groups are deferred, as in `fast-runs`.
...
- Builds on the merged `fast-runs` batching runner (`runScopeTests`, `runTestGroups`, `batchEnded`, `testRun.incomplete`). The run path adds an opt-in cancellation context and a process group for the server only; the CLI does not use them.
