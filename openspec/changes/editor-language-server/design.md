...
**Status: re-planned on 2026-09-18 against the merged batching runner (`38a0349`); proposed, awaiting approval.**
...
- **Cancellation, owned by the server.** The runner has no run context, and the CLI does not handle Ctrl-C. The server adds the smallest seam it needs:
  ...
  - When the context is set, each batch command starts in its own process group (`SysProcAttr.Setpgid`), because `go test` runs the compiled test binary as a child and killing only the `go` process would orphan it. On cancellation the server kills the group (`kill(-pgid, SIGKILL)`), the batch loop starts no further batch, and `runScopeTests` returns `errRunCancelled` before it assembles or writes evidence, so the stored file keeps its bytes.
...
`fast-runs` merged as batching only (`38a0349`). Section 6, Decision 8, and the run lenses are re-planned against it (proposed, awaiting approval): the run goes through `runScopeTests` and the batching runner; progress maps the existing observer and `batchEnded` to `$/progress`; batches that did not complete are reported; the server owns cancellation; one run per project. The requirement "Run selected tests and merge their results" no longer names job counts, execution groups, or setup and teardown. Scenario `3c56123372c1` is reworded, `40c7e3ae9243` is new, and `cbace7f5db1a` is removed.
...
Open for the re-plan of section 6 (2026-09-18), with the proposed answer:

7. **How cancellation stops the process tree.** Proposed: `SIGKILL` to the batch's process group at once, since there is no teardown to wait for. Alternative: `SIGTERM`, then `SIGKILL` after a short grace period, which lets Node flush output but makes the cancel test slower.
8. **The CLI and interruption.** Proposed: leave `stele test` as it is (no process group, no Ctrl-C handling), because `fast-runs` deferred that; the server's `cancel` context stays opt-in. Alternative: adopt it for the CLI in this change.
9. **A target declared in several scopes.** A requirement modified by a change exists in the current specifications and in the change. Proposed: a lens runs in the scope of its own heading or anchor, and a command target that several scopes declare runs once, in the first scope in scope order, so a test never runs twice in one command.
