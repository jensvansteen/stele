...
- [ ] 5.3 Run `stele annotate --all` in this repository and commit the line-1 edits separately; verify `npm run verify:self` and `npm run stele -- validate --all` report no `SPEC_ANNOTATION_*` diagnostic. Deferred: run after chore/self-verify-rc3 merges (design.md, resolved question 4).
