...
- CodeLens shows a status on requirement and scenario headings, such as `unit ✓ · e2e ✗ · stale`, combined with the rule of the execution verdict, and a summary of the linked behavior on every anchor that opens its specification. Run actions follow in a later release, after the `fast-runs` change.
